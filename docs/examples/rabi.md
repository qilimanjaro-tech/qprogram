...
compact `q[0].drive` path. The measurement name `q0/readout/m0` is
auto-allocated from the bus, and the `name="q0/readout/m0"` keyword makes it
...
`qp.simulate` runs the program on the reference software platform, which
produces the same result shapes a hardware platform's `execute()` returns:
...
- Sweep frequency instead of gain by adding a second `sweep` (or a
  parallel `|` composition). See [Control flow](../guide/control-flow.md).
- Use `fields=(qp.MeasurementField.IQ, qp.MeasurementField.RAW)` if you also
  want the raw ADC trace. See
