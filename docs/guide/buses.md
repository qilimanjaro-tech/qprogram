...
The default naming pattern is `"{element}{index}/{kind}"`, which is why
`q[0].readout` reads `"q0/readout"`.
...
schema.c[0, 1].flux  # "c0_1/flux" — a tuple index joins with "_"
```
...
schema = BusSchema.flux_tunable_transmon(naming=BusNaming("{kind}_{element}{index}_bus"))
...
```python
from qprogram.waveforms import FlatTop, IQDrag, Square
...
```python
q[0].flux_x  # AttributeError: no such bus on a flux-tunable transmon qubit
schema.resonator  # AttributeError: no such element in this schema
```
...
BusSchema.transmon_coupled().c[3, 7].flux  # "c3_7/flux"
```
...
schema_b.q[0].drive, ...)`, QProgram refuses with a `ValidationError` at the
call site. That bus would serialize fine but mean something different on load,
...
reference built from a different schema.
...
structure and the validation behavior do.
