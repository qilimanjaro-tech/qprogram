...
QProgram is a Python DSL for describing pulse-level quantum experiments. A program says what the chip should do; a platform decides how to run it. The package is the language plus everything that can be settled without an instrument attached: the AST, the `.qp` text format, the capability protocol a platform validates programs against, a reference executor written in Python, and the hooks vendor packages register themselves through. Its only runtime dependencies are `numpy` and `xarray`.
