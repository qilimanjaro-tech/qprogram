...
Four pages of normative material: the two text formats the package reads and writes, the identifiers the DSL holds back, the exception hierarchy, and the generated API listing. Where one of these pages disagrees with a guide page, this section is the one to trust. Where one of them disagrees with `src/`, the source wins, and the page is a bug.
