...
- **Blocks** are the containers. `Block` (generic), `Average`, `ForLoop`,
  `Loop`, and `Parallel`.
...
`Block.walk()` yields every descendant in source order, and
`Block.buses()` / `Block.waveforms()` / `Block.variables()` aggregate
references for the whole subtree. Use them when you need to inspect or
transform a program.
...
The four flavours of block context manager are described in
[Control flow](control-flow.md).

## Hardware and software loops look identical

QProgram makes no syntactic distinction between a hardware sweep and a
software sweep. The same `for_loop` over a `set_parameter` will run as a
software loop; the same `for_loop` over a `play` may run on the sequencer.
...
`returns` field, and access patterns.

## The four ways to read this docs
