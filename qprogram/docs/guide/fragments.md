...
body — control flow, vendor namespaces (`f.qblox.acquire(...)`), local
...
can't be parameterized (`for_loop` validates its numbers eagerly); loop
...
host (`x_pulse_n`, `x_pulse_n_2`, …); colliding measurement names gain a
suffix (`m0`, `m0_2`, …) by renaming the shared handle, so `handle.state`
conditionals inside the fragment stay consistent. Nested calls expand
