...
def _measure_with_state(p: QProgram) -> MeasurementHandle:
    return p.measure("readout_q0", _readout_wf(), _weights_wf(), returns="iq,state")
...
def test_conditional_walk_yields_self_then_arm_bodies() -> None:
    ...
    assert type_names.count("Block") == 2  # two arm bodies
    assert type_names.count("Play") == 2  # two play ops
...
def test_if_rejects_wrong_comparison_shape() -> None:
    """v1 only supports MeasurementRef == int / != int."""
    p = QProgram()
    var = p.variable("v")
...
def test_native_eq_reversed_int_first() -> None:
    ...
    h = MeasurementHandle("q0_m0")
    c = 0 == h.state  # noqa: SIM300 — deliberately yoda to verify the reverse path
    assert isinstance(c, Comparison)
...
def test_if_accepts_proxy_to_proxy() -> None:
    ...
    m2 = p.measure("readout_q1", _readout_wf(), _weights_wf(), returns="state")
...
def test_validate_with_state_returns_clean() -> None:
    p = QProgram()
    m = _measure_with_state(p)
    with p.if_(m.state == 0):
        p.play("drive_q0", "a")
