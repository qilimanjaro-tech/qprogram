"""Reference software executor — shapes, values, feedback, and the execution convention."""
...
def _unit_response(bus: str, env: dict) -> complex:  # noqa: ARG001
    return 1.0 + 2.0j
...
def test_nested_loop_dims_coords_and_for_loop_value_formula():
    p = QProgram()
    freq = p.variable("freq")
    gain = p.variable("gain")
    with p.for_loop(freq, 4e9, 5e9, 0.5e9), p.for_loop(gain, 0.0, 1.0, 0.25):
...
def test_parallel_loops_share_one_dimension_with_both_coords():
    ...
    a = p.variable("a")
    b = p.variable("b")
    with p.for_loop(a, 0.0, 1.0, 0.5) | p.for_loop(b, 10.0, 20.0, 5.0):
        p.measure("readout_q0", _ro(), "w")
    da = run(p).get("m0")
...
def test_no_loop_measurement_is_scalar():
    p = QProgram()
    p.measure("readout_q0", _ro(), "w", returns=("iq", "state"))
    result = run(p)
...
def test_average_contributes_no_dimension():
    p = QProgram()
    g = p.variable("g")
    with p.average(50), p.for_loop(g, 0.0, 1.0, 0.5):
        p.measure("readout_q0", _ro(), "w")
    da = run(p).get("m0")
    assert da.dims == ("g", "IQ")
    assert da.shape == (3, 2)
...
def test_loop_inside_average_and_average_inside_loop():
    p = QProgram()
    g = p.variable("g")
    with p.for_loop(g, 0.0, 1.0, 0.5), p.average(10):
        p.measure("readout_q0", _ro(), "w")
    assert run(p).get("m0").dims == ("g", "IQ")
...
def test_return_token_shapes():
    ...
    with p.for_loop(g, 0.0, 1.0, 0.5):
        p.measure("readout_q0", _ro(), "w", returns=("iq", "state", "raw"))
    result = run(p, model=model)
...
def test_primary_data_is_iq_when_requested_else_first_token():
    p = QProgram()
    p.measure("readout_q0", _ro(), "w", returns=("state", "iq"))
    result = run(p)
    assert result.get("m0").dims == ("IQ",)  # iq is primary even when listed second
    p2 = QProgram()
    p2.measure("readout_q0", _ro(), "w", returns=("state",))
    assert run(p2).get("m0").dims == ()  # no iq requested -> first token


def test_get_field_miss_names_available_fields():
    p = QProgram()
    p.measure("readout_q0", _ro(), "w", returns=("iq",))
    with pytest.raises(KeyError, match=r"no field 'raw'.*available: iq"):
        run(p).get("m0", field="raw")
...
def test_rabi_response_values_exact_with_zero_noise():
    model = MockMeasurementModel(response=lambda bus, env: np.sin(np.pi * env["g"] / 2) ** 2 + 0j)  # noqa: ARG005
    p = QProgram()
    g = p.variable("g")
    with p.average(100), p.for_loop(g, 0.0, 1.0, 0.25):
        p.play("drive_q0", Gaussian(amplitude=g, duration=40, sigma=8))
        p.measure("readout_q0", _ro(), "w")
    da = run(p, model=model).get("m0")
...
def test_state_is_population_under_averaging():
    model = MockMeasurementModel(p_excited=lambda bus, env: 0.5, seed=7)  # noqa: ARG005
    p = QProgram()
    with p.average(2000):
        p.measure("readout_q0", _ro(), "w", returns=("state",))
    population = float(run(p, model=model).get("m0", field="state"))
    assert 0.4 < population < 0.6  # ~Binomial(2000, 0.5) / 2000


def test_state_is_binary_without_averaging():
    model = MockMeasurementModel(p_excited=lambda bus, env: 1.0)  # noqa: ARG005
    p = QProgram()
    p.measure("readout_q0", _ro(), "w", returns=("state",))
    assert float(run(p, model=model).get("m0", field="state")) == 1.0
...
def test_seeded_noise_is_deterministic():
    def build() -> QProgram:
        p = QProgram()
        g = p.variable("g")
        with p.average(10), p.for_loop(g, 0.0, 1.0, 0.5):
            p.measure("readout_q0", _ro(), "w")
        return p

    a = run(build(), model=MockMeasurementModel(noise=0.1, seed=42)).get("m0")
    b = run(build(), model=MockMeasurementModel(noise=0.1, seed=42)).get("m0")
    assert np.array_equal(a.values, b.values)
    c = run(build(), model=MockMeasurementModel(noise=0.1, seed=43)).get("m0")
...
def test_model_env_includes_parameters_and_loop_variables():
    ...
    def response(bus: str, env: dict) -> complex:  # noqa: ARG001
    ...
    g = p.variable("g")
    with p.for_loop(g, 0.0, 1.0, 1.0):
        p.measure("readout_q0", _ro(), "w")
    run(p, model=MockMeasurementModel(response=response))
...
def test_measurement_state_drives_conditional():
    model = MockMeasurementModel(p_excited=lambda bus, env: 1.0)  # noqa: ARG005
    p = QProgram()
    m = p.measure("readout_q0", _ro(), "w", returns=("iq", "state"))
...
def test_conditional_measurement_nan_at_unexecuted_points():
    ...
    model = MockMeasurementModel(p_excited=lambda bus, env: float(env["g"] >= 0.5))  # noqa: ARG005
    p = QProgram()
    g = p.variable("g")
    with p.for_loop(g, 0.0, 1.0, 0.25):
        m = p.measure("readout_q0", _ro(), "w", returns=("iq", "state"))
        with p.if_(m.state == 1):
            p.measure("readout_q1", _ro(), "w")
    arm = run(p, model=model).get("m1").sel(IQ="I").values
    assert np.isnan(arm[:2]).all()
    assert not np.isnan(arm[2:]).any()


def test_else_arm_executes_when_no_condition_holds():
    model = MockMeasurementModel(p_excited=lambda bus, env: 0.0)  # noqa: ARG005
    p = QProgram()
    m = p.measure("readout_q0", _ro(), "w", returns=("iq", "state"))
    with p.if_(m.state == 1):
        p.measure("readout_q1", _ro(), "w")
...
def test_condition_before_measurement_raises_unassigned():
    ...
    with p.if_(cond):
        p.sync()
    p.measure("readout_q0", _ro(), "w", returns=("iq", "state"), name="never_measured_late")
...
def test_op_expressions_are_evaluated_each_iteration():
    p = QProgram()
    t = p.variable("t")
    with p.for_loop(t, 0.0, 10.0, 5.0):
        p.wait("drive_q0", t + 4)
        p.measure("readout_q0", _ro(), "w")
    run(p)  # no raise: t is bound by the loop
...
def test_set_get_parameter_round_trip_through_store():
    model = MockMeasurementModel(response=lambda bus, env: env.get("cluster.lo", -1.0) + 0j)  # noqa: ARG005
    p = QProgram()
    ...
    p.measure("readout_q0", _ro(), "w")
    assert float(run(p, model=model).get("m0").sel(IQ="I")) == 7.5
...
def test_forced_software_surfaces_as_execution_warning():
    p = QProgram()
    v = p.variable("v")
    with p.average(10), p.for_loop(v, 0.0, 1.0, 0.5):
        p.set_frequency("drive_q0", v)
        p.set_parameter("cluster", "lo", v)
        p.measure("readout_q0", _ro(), "w")
    with pytest.warns(ExecutionWarning, match="forced-software.*'lo' is swept"):
        result = run(p)
...
def test_dummy_vendor_measurement_op_executes_generically(dummy_vendor):  # noqa: ARG001
...
def test_loaded_program_executes_identically():
    def model() -> MockMeasurementModel:
        return MockMeasurementModel(
            response=lambda bus, env: env["g"] + 0j,  # noqa: ARG005
            noise=0.05,
            seed=11,
        )
    ...
    p = QProgram()
    g = p.variable("g")
    with p.average(20), p.for_loop(g, 0.0, 1.0, 0.5):
        p.measure("readout_q0", _ro(), "w")
    reloaded = qp.loads(qp.dumps(p))
    a = run(p, model=model()).get("m0")
