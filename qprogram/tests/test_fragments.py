"""Fragments: builder API, @fragment decorator, call binding, and expansion semantics."""
...
def test_fragment_builder_is_full_qprogram_surface():
    ...
    t = frag.variable("t")
    with frag.average(10), frag.for_loop(t, 0, 100, 10):
        frag.play(bus, "wf")
        frag.wait(bus, t)
        frag.sync()
...
def test_call_unsupported_argument_type():
    ...
    with pytest.raises(ValidationError, match="unsupported argument type"):
        p.call(_two_param_fragment(), True, 2)  # noqa: FBT003 — bool is exactly the rejected type
...
def test_vendor_namespace_works_on_fragment(dummy_vendor):  # noqa: ARG001
...
def test_expand_local_variable_hygiene_across_calls():
    ...
    loops = [b for b in expanded.body.walk() if type(b).__name__ == "ForLoop"]
...
def test_expand_local_variable_collision_with_host():
    frag = Fragment("scan")
    n = frag.variable("n")
    with frag.loop(n, np.asarray([1.0, 2.0])):
        frag.wait("aux", n)
    p = QProgram()
...
def test_expand_measurement_names_uniquified_with_consistent_refs():
    frag = Fragment("ro")
    bus = frag.parameter("bus")
    handle = frag.measure(bus, "wf", "w", returns=("iq", "state"))
...
def test_expand_is_deterministic_and_pure():
    ...
    amp = frag.parameter("amp")
    n = frag.variable("n")
    with frag.for_loop(n, 0, 4, 1):
        frag.set_gain("bus", amp + n)
    p = QProgram()
    g = p.variable("g")
    with p.loop(g, np.asarray([0.1, 0.2])):
        p.call(frag, g)
        p.call(frag, (g * 3))
...
def test_validate_auto_expands_calls():
    from test_validation import _full_caps  # noqa: PLC0415 — shared fixture helper
