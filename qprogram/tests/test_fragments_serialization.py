"""Fragments through the ``.qp`` text format: round-trip, strict errors, vendor requires."""
...
def test_schema_busref_call_argument_round_trips():
    ...
    from qprogram.buses import BusRef  # noqa: PLC0415
...
def test_fragment_with_vendor_op_emits_require(dummy_vendor):  # noqa: ARG001
...
def test_hand_written_file_matches_python_built_program():
    ...
    p = QProgram()
    g = p.variable("g")
    with p.average(100), p.for_loop(g, 0, 1, 0.1):
        p.call(x_pulse, "drive_q0", g)
...
def test_arbitrary_array_values_inside_fragment_round_trip():
    ...
    with frag.loop(n, values):
        frag.set_gain("bus", n)
    p = QProgram()
    p.call(frag)
    p2 = qp.loads(qp.dumps(p))
    loaded_loop = p2.fragments["sweep"].body.elements[0]
    assert np.array_equal(loaded_loop.values, values)
