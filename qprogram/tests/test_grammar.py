"""CI cross-check between the canonical ``qp.lark`` grammar and the production parser.
...
def _full_feature_program() -> QProgram:
    ...
    h = p.variable("h")
    k = p.variable("k")
    with p.for_loop(h, 0.0, 1.0, 0.5) | p.for_loop(k, 5.0, 10.0, 2.5):
        p.set_gain("drive_q0", h)
    with p.loop(t, np.asarray([1.0, 2.0, 4.0])), p.block():
        p.wait("aux", t)
    return p
...
def _schema_program() -> QProgram:
    ...
    f = p.variable("f")
    with p.for_loop(f, 4e9, 5e9, 0.5e9):
...
def _vendor_program(dummy_vendor_active: bool = True) -> QProgram:  # noqa: ARG001
    p = QProgram()
...
def test_vendor_program_is_grammatical(dummy_vendor):  # noqa: ARG001
...
_SYNTACTIC_REJECTS = {
    ...
    "unparenthesised-expression": '#!QProgram 1.0\n\nbody:\n  wait "d" 100 - t\n',
...
def test_reserved_format_keywords_match_grammar_keywords():
    """Every hard keyword the grammar declares must be reserved as an identifier in the DSL —
    otherwise `var for` would be accepted by the parser but rejected by the grammar."""
    from qprogram import RESERVED_KEYWORDS  # noqa: PLC0415
