"""WaveformLibrary — per-bus resolution and the portable ``.wfl`` text format."""
...
def _full_library() -> WaveformLibrary:
    ...
    library.set("pi_pulse", IQDrag(0.5, 40, 8, 0.1), element="q", idx=0, kind="drive")  # exact
    library.set("pi_pulse", IQDrag(0.9, 40, 8, 0.1), element="q", idx=1, kind="drive")  # exact
    library.set("cz", Square(0.3, 200), element="c", idx=(0, 1), kind="flux")  # exact, tuple idx
...
def test_get_tries_exact_then_family_then_global():
    ...
    assert library.get(q[0].drive, "pi_pulse").amplitude == 0.5  # exact q0
    assert library.get(q[1].drive, "pi_pulse").amplitude == 0.9  # exact q1
    assert library.get(c[0, 1].flux, "cz").amplitude == 0.3  # exact, tuple idx
    assert library.get(q[7].readout, "readout") is not None  # family: any index
    assert library.get("raw_bus", "weights") is not None  # global: even a raw-string bus
...
def test_raw_string_bus_reaches_only_global_tier():
    ...
    assert library.get("q0/drive", "pi_pulse") is None  # raw string can't match the exact tier
