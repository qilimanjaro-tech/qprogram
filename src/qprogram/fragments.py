...
class Fragment(QProgram):
    ...
    Args:
        name (str): Fragment identifier — must match ``[A-Za-z_][A-Za-z0-9_]*`` and not be a
            reserved keyword. Used verbatim as the ``.qp`` definition and call name.
        label (str, optional): Human-readable label. A ``.qp`` fragment section is headed by the
            name and the parameter list alone, so the label does not survive serialization.
        description (str | None): Longer free-form description. Not serialized either.
    ...
    def __eq__(self, other: object) -> bool:
        """Compare two fragments structurally.

        Args:
            other (object): The object to compare against.

        Returns:
            ``True`` when ``other`` is a fragment of the same type carrying the same name,
            parameter ids, local variables, and body.
        """
        if type(self) is not type(other):
            return False
        assert isinstance(other, Fragment)  # narrowed by the type check above  # ruff: ignore[assert]
        return (
...
def bind_arguments(frag: Fragment, args: tuple[object, ...], kwargs: dict[str, object]) -> dict[str, object]:
    ...
    Args:
        frag (Fragment): The fragment being called.
        args (tuple[object, ...]): Positional arguments, matched to parameters in declaration order.
        kwargs (dict[str, object]): Keyword arguments, matched by parameter id.
...
def expand_program(program: QProgram) -> QProgram:
    ...
    Args:
        program (QProgram): The program to lower. Never mutated.

    Returns:
        A fragment-free deep copy of ``program``. Its fragment registry and ``.qp`` source map are
        both empty, because expansion restructures the tree the recorded paths address.
    ...
    expanded._fragments = {}  # ruff: ignore[private-member-access]
    expanded._qp_source_map = {}  # ruff: ignore[private-member-access] — expansion restructures the tree; recorded paths are stale
    return expanded
...
def _expand_in_block(block: Block, host: QProgram, stack: tuple[str, ...], used_names: set[str]) -> None:
    ...
    Args:
        ...
        host (QProgram): The program the fragment-local variables are hoisted onto.
        stack (tuple[str, ...]): Names of the fragments currently being expanded, used for cycle
            detection.
        used_names (set[str]): Measurement names already taken in the host program; extended as
            fragment measurements are uniquified.
...
def _expand_call(call: Call, host: QProgram, stack: tuple[str, ...], used_names: set[str]) -> Block:
    ...
    Args:
        ...
        host (QProgram): The program that receives the freshly renamed fragment-local variables.
        stack (tuple[str, ...]): Names of the fragments currently being expanded, innermost last.
        used_names (set[str]): Measurement names already taken; extended in place.

    Returns:
        A plain block holding the substituted body, ready to take the call's place.

    Raises:
        ValidationError: If the fragment is already on ``stack`` (a call cycle), or if a binding is
...
def _fresh_variable_id(host: QProgram, base: str) -> str:
    """Return ``base``, or ``base`` with the lowest free numeric suffix if ``host`` already uses it.

    Args:
        host (QProgram): The program whose variable ids must not be shadowed.
        base (str): The preferred identifier.

    Returns:
        An identifier not yet declared on ``host``.
    """
...
def _uniquify_measurements(body: Block, used_names: set[str]) -> None:
    ...
    Args:
        ...
        used_names (set[str]): Measurement names already taken; every name this function settles on
            is added to it.
    """
...
def _substitute_in_body(body: Block, mapping: dict[str, object], frag: Fragment) -> None:
    """Substitute parameters and local variables across every node of a copied fragment body.

    Args:
        body (Block): The copied fragment body, rewritten in place.
        mapping (dict[str, object]): Parameter and local-variable ids mapped to their replacements.
        frag (Fragment): The fragment being expanded, named in error messages.

    Raises:
        ValidationError: If a loop variable ends up bound to something other than a variable, or if
            a binding is used in an incompatible position.
    """
    for node in body.walk():
        if isinstance(node, Operation):
            ...
            _substitute_node_attrs(node, mapping, frag)
            if isinstance(node, Sweep) and not isinstance(node.variable, Variable):
                msg = (
...
def _substitute_node_attrs(node: Block | Operation, mapping: dict[str, object], frag: Fragment) -> None:
    """Rewrite one node's public attributes with the substituted values.

    Args:
        node (Block | Operation): The node to rewrite in place.
        mapping (dict[str, object]): Parameter and local-variable ids mapped to their replacements.
        frag (Fragment): The fragment being expanded, named in error messages.

    Raises:
        ValidationError: If a parameter used inside an expression is bound to something that is not
            a number, variable, or expression — a bus or a waveform, say.
    """
...
def _subst_value(value: object, mapping: dict[str, object], frag: Fragment) -> object:
    ...
    Waveforms, lists, tuples, and dicts are walked so a parameter nested inside them is reached too.

    Args:
        value (object): The attribute value to rewrite.
        mapping (dict[str, object]): Parameter and local-variable ids mapped to their replacements.
        frag (Fragment): The fragment being expanded, named in error messages.

    Returns:
        The substituted value. A value ``mapping`` has nothing to say about comes back unchanged;
        a list, tuple, or dict is always rebuilt as a fresh container over substituted items, so
        identity is not preserved there.
...
def _subst_expr(expr: Expression, mapping: dict[str, object], frag: Fragment) -> Expression:
    ...
    Args:
        expr (Expression): The expression tree to rewrite.
        mapping (dict[str, object]): Parameter and local-variable ids mapped to their replacements.
        frag (Fragment): The fragment being expanded, named in error messages.
...
def _revalidate_op(op: Operation, frag: Fragment) -> None:
    ...
    Raises:
        ValidationError: If a bus attribute does not hold a bus, a waveform attribute does not hold
            a waveform or alias, the waveform does not match the bus channel, or a measurement
            targets a bus without an ADC.
    """
