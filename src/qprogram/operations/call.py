...
"""The fragment call site — the AST leaf that instantiates a :class:`~qprogram.Fragment`."""
...
class Call(Operation):
    ...
    Args:
        fragment (Fragment): The called :class:`~qprogram.Fragment`.
        arguments (dict[str, object]): Fully-bound ``{param_id: value}`` mapping (one entry per
            fragment parameter), produced by :func:`qprogram.fragments.bind_arguments`.
    """
    ...
    def buses(self) -> set[str]:
        ...
        A :class:`~qprogram.Parameter` is untyped — only the position an argument lands in inside
        ...
        :meth:`QProgram.expand` has substituted the arguments.
        """
