...
"""Tests for ``explain()`` — the execution plan rendered as a tree."""

from __future__ import annotations

from test_validation import _drag_sigma_excludes_rt, _empty_caps, _full_caps
...
def test_domain_column_per_node():
    ...
    assert "for g in Range(start=0.0, stop=1.0, step=0.1):" in out
    assert 'play "drive_q0" "pi"' in out
    assert "[rt|host]" in out
    assert "errors: 0" in out


def test_forced_host_reason_inline():
    caps = _full_caps(bus_predicates=(_drag_sigma_excludes_rt,))
    p = QProgram()
    sigma = p.variable("sigma")
    with p.average(100), p.sweep(sigma, Range(1, 10, 1)):
    ...
    # host-side-only Sweep, so its reason is structural — naming the sub-block — and carries the
    ...
    assert "[host]" in average_line
    assert "~ forced-host: contains host-side-only sub-block 'Sweep'" in average_line
...
def test_node_less_diagnostics_in_footer():
    ...
    a = p.variable("a")
    b = p.variable("b")
    with p.sweep(a, Range(0, 1, 0.5)), p.sweep(b, Range(0, 1, 0.5)):
        p.play("drive_q0", "pi")
    out = explain(p, caps)
...
def test_parallel_row_renders_pipe_joined_headers():
    ...
    a = p.variable("a")
    b = p.variable("b")
    with p.sweep(a, Range(0, 1, 0.5)) | p.sweep(b, Range(5, 10, 2.5)):
        p.set_gain("drive_q0", a)
    out = explain(p, _full_caps())
    assert "for a in Range(start=0.0, stop=1.0, step=0.5) | for b in Range(start=5.0, stop=10.0, step=2.5):" in out
...
def test_platform_explain_default_delegates():
    from qprogram import PlatformProtocol  # ruff: ignore[import-outside-top-level]
    from qprogram.buses import BusSchema  # ruff: ignore[import-outside-top-level]
    ...
    class DemoPlatform(PlatformProtocol):
        ...
        def get_parameters(self, bus):  # ruff: ignore[unused-method-argument] — abstract signature
            return []
...
def test_validate_stamps_resolvable_paths():
    ...
    warning = next(d for d in diagnostics if d.code == "forced-host")
