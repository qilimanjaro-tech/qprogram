...
"""Tests for the ``.qp`` checker (``qprogram.lsp``): ``check_text`` core, CLI modes, and the LSP server."""
...
_WARNY = (
    ...
    "\n"
    "  average 10:\n"
    "    for v in Range(start=0.0, stop=1.0, step=0.5):\n"
    '      set_frequency "d" v\n'
...
def test_validation_warning_mapped_via_source_map():
    ...
    # The average also draws a "reorderable-averaging" info hint (its measurement is
    # real-time-capable), so keep only the forced-host warning this test maps.
    warnings = [d for d in diagnostics if d.code == "forced-host"]
