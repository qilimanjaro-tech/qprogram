...
"""Tests for structural AST paths and the ``.qp`` source map."""
...
def _program() -> QProgram:
    ...
    with p.average(100), p.sweep(g, Range(0, 1, 0.1)):  # body[1] / body[1][0]
        p.wait("drive_q0", 4)  # body[1][0][0]
        m = p.measure("readout_q0", "ro", "w", fields=("iq", "state"))  # body[1][0][1]
    ...
    with p.sweep(g, Range(0, 1, 0.5)) | p.sweep(h, Range(5, 10, 2.5)):  # body[2] (Parallel)
...
def test_source_map_maps_paths_to_their_lines():
    ...
    expectations = {
        (0,): 'play "drive_q0" "pi"',
        (1,): "average 100:",
        (1, 0): "for g in Range(start=0.0, stop=1.0, step=0.1):",
...
def test_source_map_round_trips_a_diagnostic_path_to_the_offending_line():
    """A :attr:`Diagnostic.path` addresses the exact line of the serialized ``.qp`` text.

    This is the headline flow: validate a built program, then resolve each diagnostic path
    through the source map of the reparsed text.
    """
    from test_validation import _empty_caps  # ruff: ignore[import-outside-top-level] — shared fixture helper
