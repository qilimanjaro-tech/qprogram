...
# ---------------------------------------------------------------------------
# Measure — gathers tokens from waveform + weights + fields
# ---------------------------------------------------------------------------
...
def test_measure_with_concrete_waveform_and_weights() -> None:
    ...
    assert "measure.fields.iq" in caps
...
def test_measure_with_raw_return_token() -> None:
    wf = IQPair(I=Square(0.5, 100), Q=Square(0.0, 100))
    m = Measure(bus="readout_q0", waveform=wf, weights=wf, handle=MeasurementHandle("q0_m0"), fields=("iq", "raw"))
    caps = m.required_capabilities()
    assert "measure.fields.iq" in caps
    assert "measure.fields.raw" in caps
...
def test_set_parameter_picks_up_value_expr() -> None:
    v = Variable("p")
    assert SetParameter(bus="drive_q0", parameter="x", value=v).required_capabilities() == {
        "op.set_parameter",
        "expr.variable",
    }
...
def test_get_parameter_token() -> None:
    v = Variable("x")
    assert GetParameter(variable=v, bus="drive_q0", parameter="y").required_capabilities() == {
        "op.get_parameter",
    }
...
def test_range_source_emits_linear_sweep() -> None:
    v = Variable("x")
    assert Sweep(variable=v, source=Range(start=0, stop=1, step=0.1)).required_capabilities() == {
        "block.sweep",
        "sweep.linear",
        "sweep.range",
    }
...
def test_values_source_emits_arbitrary_sweep() -> None:
    v = Variable("x")
    assert Sweep(variable=v, source=Values(np.array([0.0, 0.5, 1.0]))).required_capabilities() == {
        "block.sweep",
        "sweep.arbitrary",
        "sweep.values",
    }
...
def test_parallel_emits_block_parallel_only() -> None:
    """Parallel's own token is ``block.parallel`` alone.

    The composed loops contribute their own capability tokens when the walker visits them
    separately.
    """
    v1 = Variable("a")
    v2 = Variable("b")
    p = Parallel(
        loops=[
            Sweep(variable=v1, source=Range(start=0, stop=1, step=1)),
            Sweep(variable=v2, source=Values(np.array([0.0, 1.0]))),
        ],
    )
