...
"""Tests for the end-to-end ``.qp`` round-trip across feature combinations."""
...
def test_round_trip_all_core_operations(transmon_schema):
    ...
    p.sync()
    p.set_parameter(transmon_schema.q[0].drive, "lo_frequency", 5e9)
    p.get_parameter(transmon_schema.q[0].drive, "lo_frequency")
    _assert_byte_stable(p)
...
def test_round_trip_with_range_sweep():
    p = QProgram()
    v = p.variable("freq")
    with p.sweep(v, Range(4e9, 6e9, 1e6)):
...
def test_round_trip_with_loop_values():
    p = QProgram()
    v = p.variable("amp")
    with p.sweep(v, Values(np.array([0.1, 0.3, 0.5]))):
        p.set_gain("drive", v)
    _assert_byte_stable(p)
...
def test_round_trip_with_parallel_loops():
    ...
    v = p.variable("freq")
    w = p.variable("gain")
    with p.sweep(v, Range(4e9, 6e9, 1e6)) | p.sweep(w, Range(0.0, 2.0, 0.001)):
        p.set_frequency("drive", v)
        p.set_gain("drive", w)
    _assert_byte_stable(p)
...
def test_round_trip_with_deeply_nested_blocks():
    p = QProgram()
    v = p.variable("x")
    with p.average(100), p.sweep(v, Range(0.0, 1.0, 0.1)), p.block():
        p.wait("bus", v)
    _assert_byte_stable(p)
...
def test_round_trip_with_fields_kwarg(transmon_schema):
    p = QProgram(schema=transmon_schema)
    p.measure(transmon_schema.q[0].readout, "r", "w")
    p.measure(transmon_schema.q[0].readout, "r", "w", fields=("iq", "raw"))
    _assert_byte_stable(p)
...
def test_round_trip_full_features(transmon_schema):
    ...
    with (
        ...
        p.sweep(gain, Range(0.0, 0.3, 0.1)) | p.sweep(t, Values(np.array([10, 20, 30, 40]))),
        p.sweep(freq, Range(4e9, 6e9, 1e6)),
    ):
...
def test_round_trip_with_vendor(transmon_schema, dummy_vendor):  # ruff: ignore[unused-function-argument]
    ...
    p.dummy.acquire(transmon_schema.q[0].readout, "w", fields=("iq", "raw"))
    _assert_byte_stable(p)


# ---------------------------------------------------------------------------
# Round-trip integrity: long arrays, vendor ops in arms, quoting, and names
# ---------------------------------------------------------------------------
...
def test_round_trip_long_loop_values_lossless():
    ...
    with p.sweep(v, Values(values)):
        p.set_gain("drive", v)
    reloaded = loads(dumps(p))
    lp = reloaded.body.elements[0]
    assert np.array_equal(lp.source.values(), values)
    assert reloaded.body == p.body
...
def test_round_trip_long_arbitrary_samples_lossless():
    ...
    p.play("drive", Square(0.5, 4))  # Neighbor op to keep the body realistic.
...
def test_round_trip_vendor_op_inside_conditional(transmon_schema, dummy_vendor):  # ruff: ignore[unused-function-argument]
    """Vendor ops used only inside conditional arms keep their require line and survive."""
    from _dummy_vendor import DummyQProgram, DummySetMarkers  # ruff: ignore[import-outside-top-level]

    p = DummyQProgram(schema=transmon_schema)
    h = p.measure(transmon_schema.q[0].readout, "r", "w", fields=("iq", "state"))
