...
"""Tests for the round-trip property — ``loads(dumps(p))`` equals ``p`` over generated programs.
...
re-emitted text must be byte-stable. Each strategy is deliberately adversarial about the fragile
spots: long arrays, quotes/backslashes/``#``/newlines in metadata text, path-shaped raw bus
strings, ints vs floats, and deep block nesting.
"""
...
from qprogram import Fragment, MeasurementField, QProgram, dumps, loads
from qprogram.buses import BusSchema
from qprogram.sweeps import Concat, Linspace, Logspace, Range, Repeat, Rotate, Values
...
# Free-form human text — every UTF-8 character except surrogates and control codes.
human_text = st.text(
...
# Raw-string bus names: plain identifier-shaped names, plus adversarial path-shaped strings that
# must NOT be promoted to bus references on reload.
bus_names = st.one_of(
...
def programs(draw: st.DrawFn) -> QProgram:
    ...
    def draw_sweep_source(draw):
        """Any built-in sweep source, including a nested combinator.

        Every source has to survive the round trip, so the property test covers the whole
        registry, plain sources and combinators alike.
        """
        kind = draw(st.sampled_from(["values", "linspace", "logspace", "repeat", "rotate", "concat"]))
        n = draw(st.integers(min_value=1, max_value=20))
        if kind == "values":
            return Values(np.asarray(draw(st.lists(finite_floats, min_size=n, max_size=n))))
        if kind == "linspace":
            return Linspace(draw(finite_floats), draw(finite_floats), num=n)
        if kind == "logspace":
            lo = draw(st.floats(min_value=1e-6, max_value=1e3))
            return Logspace(lo, lo * draw(st.floats(min_value=1.1, max_value=1e3)), num=n)
        inner = Values(np.asarray(draw(st.lists(finite_floats, min_size=n, max_size=n))))
        if kind == "repeat":
            return Repeat(inner, times=draw(st.integers(min_value=1, max_value=4)))
        if kind == "rotate":
            return Rotate(inner, by=draw(st.integers(min_value=-3, max_value=3)))
        return Concat([inner, Values(np.asarray(draw(st.lists(finite_floats, min_size=1, max_size=5))))])
    ...
    def emit_ops(depth: int) -> None:
        for _ in range(draw(st.integers(min_value=1, max_value=3))):
            ...
            if kind == "play":
                ...
                # A random non-empty subset in random order — the writer must emit it canonically
                # and the parser must recover the same tuple.
                fields = draw(
                    st.lists(st.sampled_from(list(MeasurementField)), min_size=1, max_size=3, unique=True),
                )
                p.measure(bus, draw(waveform_aliases), draw(waveform_aliases), fields=fields)
    ...
    def emit_block(depth: int) -> None:
        ...
        if choice == "average":
            ...
            stop = start + step * draw(st.integers(min_value=1, max_value=50))
            with p.sweep(v, Range(start, stop, step)):
                emit_ops(depth)
        elif choice == "loop" and variables:
            v = draw(st.sampled_from(variables))
            with p.sweep(v, draw_sweep_source(draw)):
                emit_ops(depth)
        else:
            with p.block():
                emit_ops(depth)
...
def test_round_trip_long_sweeps_property(values: list[float]) -> None:
    p = QProgram()
    v = p.variable("amp")
    with p.sweep(v, Values(np.asarray(values))):
        p.set_gain("drive", v)
    reloaded = loads(dumps(p))
    assert np.array_equal(reloaded.body.elements[0].source.values(), np.asarray(values))
...
def fragment_programs(draw: st.DrawFn) -> QProgram:
    ...
    if draw(st.booleans()):
        ...
        with frag.sweep(n, Values(np.asarray(draw(st.lists(finite_floats, min_size=k, max_size=k))))):
...
def test_path_shaped_raw_bus_string_survives_with_schema(raw_bus: str) -> None:
    """A quoted raw-string bus that *looks* like a path stays a plain string.

    The program carries a schema the path would resolve against, so the reload has every excuse to
    promote it to a bus reference and must not take it.
    """
