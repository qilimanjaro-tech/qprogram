...
def test_ast_eq_ndarray_vs_list():
    # With one array operand the helper takes the ndarray branch, which requires both sides to be
    # arrays before it can return True.
...
def test_ast_hash_dict_order_independent():
    # Hashing sorted items makes the result independent of insertion order.
    a = {"a": 1, "b": 2}
    b = {"b": 2, "a": 1}
    assert ast_hash(a) == ast_hash(b)
